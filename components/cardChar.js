export default class CardChar extends React.Component {
    constructor(props) {
        super(props);

        this.state = {
            showInput: false,
            name: null,
            finished: false,
            correct: false,
            points: 10
        }

        this.showInput = this.showInput.bind(this);
        this.showDetail = this.showDetail.bind(this);
        this.confirmName = this.confirmName.bind(this);
        this.getName = this.getName.bind(this);
    }

    componentWillReceiveProps() {
        if(this.props.finished)
            this.setState({ finished: this.props.finished });
    }

    showInput() {
        this.setState({ showInput: !this.state.showInput });
    }

    showDetail() {
        const { onGetDetail, info } = this.props;

        this.state.points = 5;

        onGetDetail(info);
    }

    confirmName() {
        const { name, points } = this.state;
        
        if(name.toLowerCase() == this.props.info.name.toLowerCase()) {
            this.props.onPontuation(points);
            this.setState({ finished: true, correct: true });
        }else 
            this.setState({ finished: true, correct: false });
    }

    getName(e) {
        if(e.keyCode == 13)
            this.confirmName();

        this.state.name = e.target.value;
    }

    render() {
        const { showInput, finished, correct, name } = this.state;
        return(
            <li className={`game-content-list-item ${finished ? correct ? "correct" : "wrong" : ""}`}>
                <span>{this.props.info.name}</span>
                <figure className="game-content-list-item-pic"> 
                    <img className="game-content-list-item-pic-img" src="data:image/jpeg;base64," />
                </figure>

                

                <React.Fragment>
                {
                    (finished) ? 
                        <p>{name}</p> :
                        <React.Fragment>
                        <div className="game-content-list-item-controllers">
                            <button onClick={this.showInput} className="game-content-list-item-button game-content-list-item-button-show">?</button>
                            <button onClick={this.showDetail} className="game-content-list-item-button game-content-list-item-button-response">...</button>
                        </div>
                        <div className={`game-content-list-item-name ${showInput ? "show" : ""}`}>
                            <input onKeyUp={this.getName} type="text" className="game-content-list-item-name-input" />
                            <button onClick={this.confirmName} className="game-content-list-item-name-send">Ok</button>
                        </div>
                        </React.Fragment>
                }
                </React.Fragment>
            </li>
        );
    }
}